...
//  LightPark
//
//  Created by Brian Loughran on 12/17/18.
...
class PayoutsViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        // check if the user has already set their payment
        if AppState.sharedInstance.stripeStatus {
            let alert = UIAlertController(title: "Payouts already enabled", message: "Your payouts are already enabled and you should be recieving regular payments for your spots. No action is needed at this time, however you may update your payout information if you like.", preferredStyle: UIAlertControllerStyle.alert)
            alert.addAction(UIAlertAction(title: "Return", style: UIAlertActionStyle.default, handler: self.handlePopViewController))
            alert.addAction(UIAlertAction(title: "Update", style: UIAlertActionStyle.default, handler: nil))
...
//                    Spinner.stop()
                    ...
                    STPAPIClient.shared().createToken(withBankAccount: bankAccount) { (token, error) in
                        if let error = error {
                            ...
                            let alert = UIAlertController(title: "Success", message: "We have successfully updated your payout information, you will now recieve periodic payments for your utilized parking spots", preferredStyle: UIAlertControllerStyle.alert)
                            alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.default, handler: self.handlePopViewController))
                            self.present(alert, animated: true, completion: nil)
                        }
                    }
    ...
    // go to the preceding viewcontroller
    func handlePopViewController(alert: UIAlertAction) {
